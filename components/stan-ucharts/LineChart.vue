...
export default {
	...
	mounted() {
		let defaultOption = {
			...
			fontSize: 20, //全局默认字体大小（可选，单位为px，默认13px）高分屏不必乘像素比，自动根据pixelRatio计算
			...
			xAxis: {
				...
				itemCount: 200, //X轴可见区域数据数量（即X轴数据密度），配合拖拽滚动使用（即仅在启用enableScroll时有效）
			...
			lineCharts[this.canvasId].showToolTip(e, {
				...
				format: function(item, category) {
					...
					if (typeof item.data === 'object') {
						...
						let dataValue = typeof item.data === 'object' ? item.data.value : item.data;
						if (type == 'number'&& typeof dataValue === 'number') {
